...
async function run(cmd) {
    ...
    for (const block of Object.keys(templateAnatomy[templateType]).filter(
        ...
        for (const item of Object.keys(template[block] || [])) {
            choices.push({
                name: `${item} ${template[block][item].Type ? `[${template[block][item].Type}]` : ""
                    }`,
                value: { block, item },
            });
        }
    ...
    for (const resourceIndex in resources) {
        ...
        if (
            ...
            if (shareCode) {
                ...
                if (language.startsWith("nodejs")) {
                    language = await inputUtil.autocomplete("Select language", [{ name: "JavaScript", value: "nodejs14.x" }, "TypeScript"]);
                }

                sharedFunctionHandlers[resource] = lambdaHandlerParser.buildFileName(
                    template.Globals,
                    props,
                    runtimes.find(p => p.languageName === language || p.name === language)
                );
            }
        }
    }
    ...
    try {
        ...
        for (const funcCode of Object.keys(sharedFunctionHandlers)) {
            await pushCode(sharedFunctionHandlers[funcCode], sharedFunctionHandlers[funcCode], repo, name, language);
        }

        for (const additionalFile of additionalFiles) {
            await pushCode(additionalFile, additionalFile.replace(process.cwd(), ""), repo, name, language);
        }
        console.log(
...
async function pushCode(file, relativeFilePath, repo, name, language) {
    ...
    await githubUtil.putContent(
        ...
        repo.repo,
        repo.branch,
        path.join(repo.root, name, repo.relativePath, language, relativeFilePath),
        code
    );
