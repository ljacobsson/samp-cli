...
async function run(cmd) {
  if (!fs.existsSync(cmd.template)) {
    console.log(`Can't find ${cmd.template}. Use -t option to specify template filename`);
    return;
  }
  const ownTemplate = parser.parse("own", fs.readFileSync(cmd.template));

  const repoRoot = await github.repos.getContent({ owner, repo });
  const patterns = repoRoot.data
    .filter((p) => p.type === "dir" && /[a-zA-Z]/.test(p.name[0]))
    .map((p) => {
  ...
  const templateFile = await github.repos.getContent({
    owner,
    repo,
    path: `/${pattern.name}/template.yaml`,
  });
  ...
  for (const block of blocks) {
    ...
    if (ownTemplate[block.section][block.name]) {
        block.name = await inputUtil.text(`Naming conflict for ${block.name}. Please select a new name. Make sure to update it dependents to the new name`, `${block.name}_2`)
    }
  ...
  console.log(`${cmd.template} updated with ${pattern.name} pattern. See https://serverlessland.com/patterns/${pattern.name} for more information`);
